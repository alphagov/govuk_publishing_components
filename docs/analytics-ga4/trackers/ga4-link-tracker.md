...
The `data-ga4-track-links-only` attribute ensures that the code only attempts to track child `<a>` tag clicks within an element. Without this attribute link tracking would still work as expected, but any click on something that isn't an `<a>` tag within the element would fire our tracking JS and ultimately fail, which would be inefficient.

More specifically, clicking something other than an `<a>` would fire a JS click listener event, causing our code to traverse up the DOM to find the parent `data-ga4-link` attribute. It would then parse the `data-ga4-link` JSON but cancel the tracking process when it realises that the clicked element is not an `<a>` tag, and that there's no `url` override attribute in the JSON (`url` existing in a JSON is used to support `<button>` clicks on this tracker, as these elements don't have a `href` in HTML, so the JSON must provide them). This process would waste computing resource, so using `data-ga4-track-links-only` makes the code run more efficiently for users as it cancels the tracking process much earlier in the code when the clicked element is not an `<a>` tag.
