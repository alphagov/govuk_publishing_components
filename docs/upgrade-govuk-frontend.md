...
Run `yarn upgrade govuk-frontend@[VERSION] --exact` eg. `yarn upgrade govuk-frontend@6.1.0 --exact`. Double check `yarn.lock` to ensure the intended (instead of the latest available) version of govuk-frontend is installed.
