(function (Modules) {
  ...
  // Searches previous element siblings to find one with the same tag as set in `elementType`.
  const closestPrevious = function (element, elementType) {
    if (!element) {
      return null
    }
    ...
    while (sibling) {
      if (sibling.tagName.toLowerCase() === elementType.toLowerCase()) {
        return sibling
      }
  ...
  SuperNavigationMegaMenu.prototype.init = function () {
    ...
    // As the toggle buttons are now selectable, we should prevent the fallback
    // links from being selectable to avoid confusion.
    for (const element of this.hiddenButtons) {
      element.removeAttribute('hidden')

      const closestSiblingLink = closestPrevious(element, 'a')
