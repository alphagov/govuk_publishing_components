...
(function (Modules) {
  class AppPromoBanner {
    constructor ($module) {
      this.$module = $module
      this.$closeLink = this.$module.querySelector('.js-close-app-promo-banner')
      this.APP_PROMO_BANNER = 'app_promo_banner'
    }
    ...
    init () {
      ...
      // Do not display the cookie banner if the cookie banner is visible
      if (!this.isCookieBannerHidden()) return

      // Do not display the banner if it was previously dismissed
    ...
    /**
     * Starts the app banner module by adding an event listener to the
     * close link and showing the banner
     *
     */
    startModule () {
      if (!this.$closeLink) return

      this.$closeLink.addEventListener('click', (event) => this.handleClick(event))
    ...
    isCookiesAndCategoryPermitted () {
      ...
      const cookieCategory = window.GOVUK.getCookieCategory(this.APP_PROMO_BANNER)

      return (cookieConsent && cookieConsent[cookieCategory])
    }

    /**
     * Checks if the cookie banner is hidden
     *
     * @returns Boolean
     */
    isCookieBannerHidden () {
      const cookieBanner = document.querySelector('#global-cookie-message')
      return cookieBanner.hidden
    }
    ...
    setAppBannerCookie (isAppBannerClosed) {
      ...
      window.GOVUK.setCookie(this.APP_PROMO_BANNER, appBannerCookieValue, { days: 30 })
    }

    /**
     * Return the user's cookie settings for the app promo banner
     *
     * If the cookie is malformed, or not present,
     * returns null.
     *
     * @returns {AppBannerCookie | null} App promo banner settings
     */
    getAppPromoBannerCookieContent () {
      const appBannerCookie = window.GOVUK.getCookie(this.APP_PROMO_BANNER)

      return appBannerCookie ? JSON.parse(appBannerCookie) : null
    }

    /**
     * Handle the click of the close banner link
     *
