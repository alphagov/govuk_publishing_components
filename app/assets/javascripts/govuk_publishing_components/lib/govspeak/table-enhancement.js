  ...
  TableEnhancement.prototype.init = function () {
    ...
    for (var j = 0; j < this.allTables.length; j++) {
      ...
      tableControls.innerHTML = `
        <button class="js-toggle-container govuk-button govuk-button--secondary" data-controls="${wrapperId}" data-toggle="modal" data-target="" data-button="${j}">Open table in a popup</button>
      `
      parent.insertBefore(tableControls, tableWrapper)
      triggerElements.push(tableControls.querySelector('.js-toggle-container'))
    }

    const modal = document.getElementById('modal-for-tables')
    const modalContent = document.getElementById('modal-content')
    var module = new GOVUK.Modules.ModalDialogue(modal)
    module.init()
