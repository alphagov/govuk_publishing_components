module GovukPublishingComponents
  class ApplicationsPage
    ...
    def gem_version
      parse_file(@gemfile, /govuk_publishing_components \(([^)>=~ ]+)\)/)
    end

    def slimmer_version
      parse_file(@gemfile, /slimmer \(([^)>=~ ]+)\)/)
    end

  private

    def get_gemfile
      @exists ? get_gemfile_local : get_gemfile_remote
    end
    ...
    def get_gemfile_remote
      url = "https://raw.githubusercontent.com/alphagov/#{@application}/main/Gemfile.lock"
