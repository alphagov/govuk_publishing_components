...
RSpec.describe GovukPublishingComponents::AppHelpers::TaxonomySidebar do
  describe '#sidebar' do
    ...
    context 'given a content item tagged to taxons and with related items' do
      ...
      it 'returns a sidebar hash containing a sorted list of parent taxons and related content' do
        expect(GovukPublishingComponents::AppHelpers.configuration.statsd).to receive(
          :increment
        ).with(
    ...
    context 'when Rummager raises an exception' do
      ...
      before(:each) do
        ...
        GovukPublishingComponents::AppHelpers.configure do |config|
          config.error_handler = error_handler
        end

        expect(GovukPublishingComponents::AppHelpers.configuration.statsd).to_not receive(
          :increment
        )
      end
  ...
  def sidebar_for(content_item)
    GovukPublishingComponents::AppHelpers::NavigationHelper.new(content_item).taxonomy_sidebar
  end
