...
RSpec.describe GovukPublishingComponents::Presenters::CheckboxesHelper do
  ...
  describe "#initialize" do
    ...
    it "sets a default id" do
      ...
      box = described_class.new({ items: items })
      expect(box.id).to eq("checkboxes-1234")
    end
  ...
  describe "#should_have_fieldset" do
    ...
    it "does not have a fieldset if the heading is an empty string" do
      box = described_class.new({ items: one_checkbox, heading: "" })
      expect(box.should_have_fieldset).to be(false)
    end

    it "does not have a fieldset if the heading is a string without content" do
      box = described_class.new({ items: one_checkbox, heading: " " })
      expect(box.should_have_fieldset).to be(false)
    end
  end
