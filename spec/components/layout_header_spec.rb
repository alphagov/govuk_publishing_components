...
describe "Layout header", type: :view do
  ...
  it "has the default logo link when no logo_link is specified" do
    ...
    assert_select ".govuk-header__homepage-link[href='/']"
  end

  it "has a custom logo link when specified" do
    render_component({ logo_link: "https://example.com" })

    assert_select ".govuk-header__homepage-link[href='https://example.com']"
  end

  it "renders the signout link when requested" do
    render_component(signout_link: true)

    assert_select ".gem-c-layout-header__signout-link[href='/auth/gds/sign_out']", text: "Sign out"
  end
end
