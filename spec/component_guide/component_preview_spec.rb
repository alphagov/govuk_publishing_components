...
describe "Component preview", :capybara do
  ...
  it "can render a gem component with only its own CSS by default" do
  ...
  it "can render an app component with only its own CSS by default" do
    visit "/component-guide/app_component/preview"
    expect(page).to have_css("link[rel=stylesheet]", visible: :hidden, count: 3)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/component_guide/application-']", visible: :hidden)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/application-']", visible: :hidden)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/components/_app-component-']", visible: :hidden)
  end

  it "can render a gem component with every other components CSS in the <head>, with the component rendering last" do
    visit "/component-guide/emergency_banner/preview?all_components=true"
    expect(page).to have_css("link[rel=stylesheet][href*='/assets/govuk_publishing_components/components/']", visible: :hidden, count: 87)
  ...
  it "can render a gem component with every other components CSS in the <head>, with the component rendering first" do
  ...
  it "can render an app component with every other components CSS in the <head>, with the component rendering last" do
    visit "/component-guide/app_component/preview?all_components=true"
    expect(page).to have_css("link[rel=stylesheet][href*='/assets/govuk_publishing_components/components/']", visible: :hidden, count: 87)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/govuk_publishing_components/components/_accordion-']", visible: :hidden)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/govuk_publishing_components/components/_select-']", visible: :hidden)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/govuk_publishing_components/components/_warning-text-']", visible: :hidden)
    ...
    last_link = find("link[rel=stylesheet]:last-of-type", visible: :hidden)
    expect(last_link[:href]).to match(/\/assets\/components\/_app-component-.*\.css/)
  end

  it "can render an app component with every other components CSS in the <head>, with the component rendering first" do
    visit "/component-guide/app_component/preview?all_components=true&render_component_first=true"
    expect(page).to have_css("link[rel=stylesheet][href*='/assets/govuk_publishing_components/components/']", visible: :hidden, count: 87)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/govuk_publishing_components/components/_accordion-']", visible: :hidden)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/govuk_publishing_components/components/_select-']", visible: :hidden)
    expect(page).to have_css("link[rel='stylesheet'][href*='/assets/govuk_publishing_components/components/_warning-text-']", visible: :hidden)
    ...
    first_link = find("link[rel=stylesheet]:nth-of-type(2)", visible: :hidden)
    expect(first_link[:href]).to match(/\/assets\/application-.*\.css/)

    third_link = find("link[rel=stylesheet]:nth-of-type(3)", visible: :hidden)
    expect(third_link[:href]).to match(/\/assets\/components\/_app-component-.*\.css/)
  end
