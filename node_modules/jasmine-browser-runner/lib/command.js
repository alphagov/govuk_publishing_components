...
const subCommands = [
  ...
  },
  {
    ...
    options: commonOptions.concat([
      ...
      {
        name: 'list-not-applicable',
        type: 'bool',
        description: 'list all specs that are marked not applicable',
      },
      {
        name: 'filter',
        type: 'string',
...
class Command {
  ...
  async runSpecs(options) {
    ...
    if (options['list-not-applicable'] !== undefined) {
      config.consoleReporter = {
        ...config.consoleReporter,
        listNotApplicableSpecs: options['list-not-applicable'],
      };
    }
