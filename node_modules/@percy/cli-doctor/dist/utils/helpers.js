...
import { sectionHeader, checkLine, suggestionList, renderFindings, sectionStatus, print } from './reporter.js';
...
export const PERCY_DOMAINS = new Set(['percy.io', 'www.browserstack.com', 'hub.browserstack.com']);
...
export function captureProxyEnv() {
  ...
  const otherKeys = ['NO_PROXY', 'no_proxy', 'NODE_TLS_REJECT_UNAUTHORIZED', 'NODE_EXTRA_CA_CERTS', 'PERCY_BROWSER_EXECUTABLE'];
  const out = {};
...
export async function runConnectivityAndSSL(proxyUrl, timeout = 10000) {
  ...
  try {
    ({
      ...
      sslFindings
    } = await connectivityChecker.checkConnectivityAndSSL({
      proxyUrl,
      timeout
    }));
  } catch (err) {
...
export async function runProxyCheck(timeout = 10000) {
  ...
  let proxyFindings = [];
  try {
    proxyFindings = await proxyDetector.detectProxy({
      timeout,
      testProxy: true
    });
  } catch (err) {
...
export async function runPACCheck() {
  ...
  if (actionablePac) {
    ...
    print(log, suggestionList([`Add to your CI environment: HTTPS_PROXY=${actionablePac.detectedProxyUrl}`, `Or run: percy doctor --proxy-server ${actionablePac.detectedProxyUrl}`]));
  }
  return {
    pac: {
...
export async function runBrowserCheck(targetUrl = 'https://percy.io', proxyUrl = null, timeout = 10000) {
  ...
  try {
    ...
    if (!browserResult.chromePath) {
      ...
      print(log, suggestionList(['Install Google Chrome or set PERCY_BROWSER_EXECUTABLE to the path of a Chromium-based browser.', 'npm install @percy/cli will install a bundled Chromium.']));
    } else {
      for (const note of browserResult.notes ?? []) {
        var _note$suggestions;
        /* istanbul ignore next */
        print(log, checkLine(note.status ?? 'info', note.message));
        /* istanbul ignore next */
        if ((_note$suggestions = note.suggestions) !== null && _note$suggestions !== void 0 && _note$suggestions.length) {
          /* istanbul ignore next */
  ...
  return {
    browser: browserResult ? {
      status: browserResult.error ? 'warn' : sectionStatus((browserResult.domainSummary ?? []).filter(d => PERCY_DOMAINS.has(d.hostname)).map(d => ({
        status: d.status
      }))),
      ...
      status: 'skip',
      error: 'Browser analysis not attempted'
    }
  };
}
...
export async function runDiagnostics({
  ...
  /* istanbul ignore next */
  if (mode !== 'quick') {
    var _phase1Results$0$reas, _phase1Results$1$reas;
    const phase1Results = await Promise.allSettled([runConfigCheck(), runEnvAuditCheck()]);
    /* istanbul ignore next */
    report.checks.config = phase1Results[0].status === 'fulfilled' ? phase1Results[0].value.config : {
      status: 'fail',
      findings: [{
        status: 'fail',
        message: (_phase1Results$0$reas = phase1Results[0].reason) === null || _phase1Results$0$reas === void 0 ? void 0 : _phase1Results$0$reas.message
      }]
    };
    /* istanbul ignore next */
    report.checks.envAudit = phase1Results[1].status === 'fulfilled' ? phase1Results[1].value.envAudit : {
      status: 'fail',
      findings: [{
        status: 'fail',
        message: (_phase1Results$1$reas = phase1Results[1].reason) === null || _phase1Results$1$reas === void 0 ? void 0 : _phase1Results$1$reas.message
      }]
    };
  }
  ...
  const {
    connectivity,
    ssl
  } = await runConnectivityAndSSL(proxyUrl, parsedTimeout);
  ...
  /* istanbul ignore next */
  if (mode !== 'quick') {
    var _find, _discoveredProxies$;
    const {
      proxy
    } = await runProxyCheck(parsedTimeout);
    report.checks.proxy = proxy;
    /* istanbul ignore next */
    const discoveredProxies = (proxy.findings ?? []).filter(f => {
      var _f$proxyValidation;
      return f.proxyUrl && ((_f$proxyValidation = f.proxyValidation) === null || _f$proxyValidation === void 0 ? void 0 : _f$proxyValidation.status) === 'pass';
    }).map(f => ({
      url: f.proxyUrl,
      source: f.source
    }));
    ...
    /* istanbul ignore next */
    const pacResolvedProxy = ((_find = (pac.findings ?? []).find(f => f.detectedProxyUrl)) === null || _find === void 0 ? void 0 : _find.detectedProxyUrl) ?? null;
    bestProxy = proxyUrl || ((_discoveredProxies$ = discoveredProxies[0]) === null || _discoveredProxies$ === void 0 ? void 0 : _discoveredProxies$.url) || pacResolvedProxy || null;
  }
  ...
  if (mode !== 'quick') {
    const {
      browser
    } = await runBrowserCheck(targetUrl, bestProxy, parsedTimeout);
    report.checks.browser = browser;
  }
  const hasFail = Object.values(report.checks).some(c => (c === null || c === void 0 ? void 0 : c.status) === 'fail');
  const hasWarn = Object.values(report.checks).some(c => (c === null || c === void 0 ? void 0 : c.status) === 'warn');
  return {
...
export function _renderBrowserResults(log, browserResult, proxyUrl) {
  ...
  if (allRows.length) {
    ...
    if (hasProxy) {
      ...
      for (const d of allRows) {
        var _d$direct, _d$viaProxy;
        const directLabel = d.direct ? d.direct.reachable ? 'reachable' : 'failed' : '—';
        const proxyLabel = d.viaProxy ? d.viaProxy.reachable ? 'reachable' : d.viaProxy.errors.some(e => /PROXY_AUTH|407/i.test(e)) ? 'auth-required' : d.viaProxy.errors.some(e => /ERR_PROXY_CONNECTION_FAILED|ERR_TUNNEL_CONNECTION/i.test(e)) ? 'no-proxy' : d.viaProxy.errors.some(e => /ERR_CERT|ERR_SSL|CERTIFICATE|TLS/i.test(e)) ? 'cert-error' : d.viaProxy.blocked ? 'blocked' : d.viaProxy.sampleStatus != null && d.viaProxy.sampleStatus >= 400 ? `http-${d.viaProxy.sampleStatus}` : 'failed' : '—';
        const httpCode = String(((_d$direct = d.direct) === null || _d$direct === void 0 ? void 0 : _d$direct.sampleStatus) ?? ((_d$viaProxy = d.viaProxy) === null || _d$viaProxy === void 0 ? void 0 : _d$viaProxy.sampleStatus) ?? '—');
        const icon = {
          pass: '\u2714',
          warn: '\u26a0',
          fail: '\u2716',
      ...
      print(log, `  ${'─'.repeat(62)}`);
      for (const d of allRows) {
        var _d$direct2, _d$direct3;
        const statusLabel = (_d$direct2 = d.direct) !== null && _d$direct2 !== void 0 && _d$direct2.reachable ? 'reachable' : 'blocked';
        const httpCode = String(((_d$direct3 = d.direct) === null || _d$direct3 === void 0 ? void 0 : _d$direct3.sampleStatus) ?? '—');
        const icon = {
          pass: '\u2714',
          warn: '\u26a0',
          fail: '\u2716',
  ...
  if ((_browserResult$proxyH = browserResult.proxyHeaders) !== null && _browserResult$proxyH !== void 0 && _browserResult$proxyH.length) {
    print(log, '');
  ...
  if (!allRows.length) {
    ...
    print(log, checkLine('fail', `Percy domain(s) unreachable from Chrome: ${criticalBlocked.map(d => d.hostname).join(', ')}`));
    print(log, suggestionList([proxyUrl ? 'Ensure the proxy allows outbound HTTPS to percy.io and browserstack.com.' : 'Set HTTPS_PROXY if you are behind a corporate proxy.', 'Contact your network team to whitelist: percy.io, www.browserstack.com, hub.browserstack.com.']));
  }
}
...
export async function runAuthCheck(ctx = {}) {
  ...
  return runSection('Token Authentication', 'auth', () => authModule.checkAuth({
    proxyUrl: ctx.bestProxy,
    timeout: ctx.timeout
  }));
}
...
export async function runEnvAuditCheck() {
  const {
    checkEnvAndCI
  } = await import('../checks/env-audit.js');
