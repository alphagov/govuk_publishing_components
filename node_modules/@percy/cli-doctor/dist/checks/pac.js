 ...
 */
var _PACDetector_brand = /*#__PURE__*/new WeakSet();
export class PACDetector {
  constructor() {
    // ─── macOS ────────────────────────────────────────────────────────────────────
    _classPrivateMethodInitSpec(this, _PACDetector_brand);
  }
  /**
  ...
  async detectPAC() {
    ...
    if (process.env.PERCY_PAC_FILE_URL) {
      discovered.push({
        url: process.env.PERCY_PAC_FILE_URL,
        source: 'env:PERCY_PAC_FILE_URL'
      });
    }
    const platform = os.platform();
    if (platform === 'darwin') discovered.push(..._assertClassBrand(_PACDetector_brand, this, _macOSPacUrls).call(this));else if (platform === 'linux') discovered.push(..._assertClassBrand(_PACDetector_brand, this, _linuxPacUrls).call(this));else if (platform === 'win32') discovered.push(..._assertClassBrand(_PACDetector_brand, this, _windowsPacUrls).call(this));
    discovered.push(..._assertClassBrand(_PACDetector_brand, this, _chromePacUrls).call(this));
    discovered.push(..._assertClassBrand(_PACDetector_brand, this, _firefoxPacUrls).call(this));
    ...
    for (const {
      ...
      const finding = await _assertClassBrand(_PACDetector_brand, this, _evaluatePac).call(this, pacUrl, source);
...
function _macOSPacUrls() {
  ...
  for (const iface of interfaces) {
    try {
      ...
      if (urlMatch && (enabledMatch === null || enabledMatch === void 0 || (_enabledMatch$ = enabledMatch[1]) === null || _enabledMatch$ === void 0 ? void 0 : _enabledMatch$.toLowerCase()) === 'yes') {
        const url = urlMatch[1].trim();
        urls.push({
          url,
          source: `macOS:networksetup(${iface})`
        });
        break;
      }
  ...
  const plistPaths = ['/Library/Preferences/SystemConfiguration/preferences.plist', path.join(os.homedir(), 'Library/Preferences/com.apple.systempreferences.plist')];
...
function _windowsPacUrls() {
  const urls = [];
  try {
    const out = cp.execSync('reg query "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings" /v AutoConfigURL 2>nul', {
      timeout: 5000,
      encoding: 'utf8'
    });
...
function _chromePacUrls() {
  ...
  const localStateLocations = {
    darwin: [path.join(os.homedir(), 'Library/Application Support/Google/Chrome/Local State'), path.join(os.homedir(), 'Library/Application Support/Chromium/Local State')],
    linux: [path.join(os.homedir(), '.config/google-chrome/Local State'), path.join(os.homedir(), '.config/chromium/Local State')],
    win32: [path.join(os.homedir(), 'AppData/Local/Google/Chrome/User Data/Local State'), path.join(os.homedir(), 'AppData/Local/Chromium/User Data/Local State')]
  }[platform] ?? [];
  for (const localStatePath of localStateLocations) {
    ...
    try {
      ...
      if (fs.existsSync(prefsPath)) {
        var _prefs$proxy, _prefs$chromeos;
        const prefs = JSON.parse(fs.readFileSync(prefsPath, 'utf8'));
        const pacScript = (prefs === null || prefs === void 0 || (_prefs$proxy = prefs.proxy) === null || _prefs$proxy === void 0 ? void 0 : _prefs$proxy.pac_url) || (prefs === null || prefs === void 0 || (_prefs$chromeos = prefs.chromeos) === null || _prefs$chromeos === void 0 || (_prefs$chromeos = _prefs$chromeos.proxy) === null || _prefs$chromeos === void 0 ? void 0 : _prefs$chromeos.pac_url) || findInObject((prefs === null || prefs === void 0 ? void 0 : prefs.proxy) ?? {}, 'pac_url');
        if (pacScript) {
          urls.push({
            url: pacScript,
            source: `chrome:Preferences(${path.dirname(prefsPath)})`
          });
        }
      }
      const extProxies = findInObject(state, 'pac_url') ?? findInObject(state, 'pac_script');
      if (extProxies) urls.push({
        url: extProxies,
...
function _firefoxPacUrls() {
  ...
  let profileEntries = [];
  try {
    profileEntries = fs.readdirSync(profileDirs, {
      withFileTypes: true
  ...
  for (const profileDir of profileEntries) {
    ...
    try {
      ...
      if (typeMatch && ['2', '4'].includes(typeMatch[1]) && pacMatch) {
        ...
        urls.push({
          url: pacMatch[1],
          source: `firefox:prefs.js(${profileDir})`
        });
      }
    } catch {/* ignore */}
  }
...
async function _evaluatePac(pacUrl, source) {
  ...
  let pacScript;
  try {
    pacScript = await _assertClassBrand(_PACDetector_brand, this, _fetchText).call(this, pacUrl);
  } catch (err) {
    return {
      ...baseFinding,
      status: 'warn',
      message: `PAC file detected at ${pacUrl} (source: ${source}) but could not be fetched: ${err.message}`,
      suggestions: ['Ensure the PAC server is reachable from this machine.', `Manually inspect: curl -v "${pacUrl}"`, `Set PERCY_PAC_FILE_URL=${pacUrl} if this PAC file is accessible from your CI environment.`]
    };
  }
  ...
  try {
    ...
    return {
      ...
      suggestions: ['Manually evaluate the PAC file to confirm which proxy is used for percy.io.', `Inspect PAC content: curl "${pacUrl}"`]
    };
  }
  ...
  const finding = {
    ...
    message: isDirect ? `PAC file (${source}) resolves percy.io as: DIRECT (no proxy needed).` : `PAC file (${source}) resolves percy.io proxy as: ${resolvedProxy}`
  };
  if (!isDirect) {
    const proxyMatch = resolvedProxy.match(/PROXY\s+(\S+)/i);
    if (proxyMatch) {
      finding.detectedProxyUrl = `http://${proxyMatch[1]}`;
      finding.suggestions = [`Set HTTPS_PROXY=http://${proxyMatch[1]} in your environment so Percy can route traffic.`, `Set PERCY_PAC_FILE_URL=${pacUrl} to have Percy automatically use this PAC file.`, `Add to your CI environment: HTTPS_PROXY=http://${proxyMatch[1]}`];
    } else {
      finding.suggestions = [`Set PERCY_PAC_FILE_URL=${pacUrl} to have Percy automatically use this PAC file.`];
    }
  }
  return finding;
}
...
export function runPacScript(script, url, host) {
  ...
  Object.defineProperties(sandbox, {
    ...
    dnsDomainIs: {
      value: (h, d) => h.endsWith(d),
      enumerable: true
    },
    localHostOrDomainIs: {
      value: (h, hd) => h === hd || h.split('.')[0] === hd.split('.')[0],
      enumerable: true
    },
    isResolvable: {
      value: () => true,
  ...
  try {
    ...
    /* istanbul ignore next */
    if ((err === null || err === void 0 ? void 0 : err.code) === 'ERR_SCRIPT_EXECUTION_TIMEOUT') {
