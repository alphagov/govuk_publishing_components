 ...
 */
export async function checkEnvAndCI({
  monitoringInstance,
  percyEnv: percyEnvArg
} = {}) {
  var _systemInfo;
  const findings = [];
  ...
  function getPercyEnvFromProcess() {
    return Object.fromEntries(Object.entries(process.env).filter(([k]) => k.startsWith('PERCY_') && !k.toLowerCase().includes('token')));
  }
  const percyEnvObj = ((_systemInfo = systemInfo) === null || _systemInfo === void 0 ? void 0 : _systemInfo.percyEnvs) ?? getPercyEnvFromProcess();
  ...
  const allSetVarNames = hasToken ? ['PERCY_TOKEN', ...nonTokenPercyVarNames] : nonTokenPercyVarNames;

  // env_system_info: System info (sourced from monitoring.logSystemInfo return value)
  /* istanbul ignore if */
  if (systemInfo) {
    var _systemInfo$memory, _systemInfo$cpu, _systemInfo$os, _systemInfo$os2;
    const memGb = ((_systemInfo$memory = systemInfo.memory) === null || _systemInfo$memory === void 0 ? void 0 : _systemInfo$memory.totalGb) != null ? systemInfo.memory.totalGb.toFixed(1) : 'N/A';
    const cores = ((_systemInfo$cpu = systemInfo.cpu) === null || _systemInfo$cpu === void 0 ? void 0 : _systemInfo$cpu.cores) ?? 'N/A';
    const platform = ((_systemInfo$os = systemInfo.os) === null || _systemInfo$os === void 0 ? void 0 : _systemInfo$os.platform) ?? process.platform;
    const release = ((_systemInfo$os2 = systemInfo.os) === null || _systemInfo$os2 === void 0 ? void 0 : _systemInfo$os2.release) ?? 'N/A';
    findings.push({
  ...
  if (process.env.PERCY_PARALLEL_TOTAL) {
    ...
    if (!Number.isInteger(val) || val <= 0) {
      findings.push({
        ...
        suggestions: ['Set PERCY_PARALLEL_TOTAL to a positive integer (e.g., 4).', 'This controls how many parallel Percy build shards to expect.']
      });
    }
  }

  // env_manual_overrides: Warn about manual overrides
  const overrideVars = ['PERCY_COMMIT', 'PERCY_BRANCH', 'PERCY_PULL_REQUEST', 'PERCY_TARGET_COMMIT', 'PERCY_TARGET_BRANCH'];
  ...
  if (process.env.NODE_TLS_REJECT_UNAUTHORIZED === '0') {
    findings.push({
      ...
      suggestions: ['This disables ALL SSL certificate validation for this process.', 'Use NODE_EXTRA_CA_CERTS=/path/to/ca.crt for a safer permanent fix.', 'Only use NODE_TLS_REJECT_UNAUTHORIZED=0 for temporary debugging.']
    });
  }
  ...
  if (!commit) {
    findings.push({
      ...
      suggestions: ['Percy needs a commit SHA for baseline comparison.', 'Set PERCY_COMMIT=<sha> as a fallback.']
    });
  } else {
    findings.push({
  ...
  if (process.env.PERCY_PARALLEL_TOTAL) {
    if (!process.env.PERCY_PARALLEL_NONCE) {
      findings.push({
        ...
        suggestions: ['Both PERCY_PARALLEL_TOTAL and PERCY_PARALLEL_NONCE must be set for parallel builds.', 'The nonce should be unique per build run (e.g., CI build number).']
      });
    } else {
      findings.push({
  ...
  try {
    ...
    if (process.env.PERCY_SKIP_GIT_CHECK === 'true') {
      ...
      findings.push({
        ...
        suggestions: ['Percy uses git to detect commit and branch information.', 'Install git or set PERCY_COMMIT and PERCY_BRANCH manually.', 'Or set PERCY_SKIP_GIT_CHECK=true to suppress this warning.']
      });
    }
  }
  return findings;
