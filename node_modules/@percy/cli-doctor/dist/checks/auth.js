...
function sanitizeError(msg) {
  var _process$env$PERCY_TO;
  /* istanbul ignore next */
  if (!msg) return msg;
  let sanitized = msg.replace(/Token token=[^\s,;)']*/gi, 'Token token=***');
  /* istanbul ignore next */
  const token = (_process$env$PERCY_TO = process.env.PERCY_TOKEN) === null || _process$env$PERCY_TO === void 0 ? void 0 : _process$env$PERCY_TO.trim();
  /* istanbul ignore next */
...
export async function checkAuth(options = {}) {
  var _process$env$PERCY_TO2;
  const {
    proxyUrl,
    timeout = 10000,
    apiBaseUrl = 'https://percy.io'
  } = options;
  const findings = [];
  const token = (_process$env$PERCY_TO2 = process.env.PERCY_TOKEN) === null || _process$env$PERCY_TO2 === void 0 ? void 0 : _process$env$PERCY_TO2.trim();
  ...
  if (!token) {
    findings.push({
      ...
      suggestions: ['Set PERCY_TOKEN in your environment: export PERCY_TOKEN=<your-token>', 'Get your token from: https://percy.io → Project Settings → API Token', 'In CI, add PERCY_TOKEN as a secret environment variable.']
    });
    return findings;
  }
  ...
  try {
    const result = await httpProber.probeUrl(`${apiBaseUrl}/api/v1/tokens/details`, {
      proxyUrl,
      timeout,
      method: 'GET',
      headers: {
    ...
    if (httpStatus === 200) {
      ...
      let role = 'unknown';
      try {
        var _body$data, _body$data2;
        /* istanbul ignore next */
        const body = typeof result.body === 'string' ? JSON.parse(result.body)
        /* istanbul ignore next */ : result.body;
        /* istanbul ignore next */
        projectTokenType = (body === null || body === void 0 || (_body$data = body.data) === null || _body$data === void 0 || (_body$data = _body$data.attributes) === null || _body$data === void 0 ? void 0 : _body$data['project-token-type']) ?? 'unknown';
        /* istanbul ignore next */
        role = (body === null || body === void 0 || (_body$data2 = body.data) === null || _body$data2 === void 0 || (_body$data2 = _body$data2.attributes) === null || _body$data2 === void 0 ? void 0 : _body$data2.role) ?? 'unknown';
      } catch {
      ...
      findings.push({
        ...
        suggestions: ['Your token may be expired, revoked, or incorrectly copied.', 'Get a new token from: https://percy.io → Project Settings → API Token', 'Ensure the full token is set (no truncation from copy-paste).']
      });
      ...
      findings.push({
        ...
        suggestions: ['The token may exist but is not authorized for this operation.', 'Verify you are using the correct token for your project.', 'Get your token from: https://percy.io → Project Settings → API Token']
      });
      ...
      findings.push({
        ...
        suggestions: ['This may be a network issue rather than a token issue.', 'See the Connectivity and Proxy sections for network diagnostics.']
      });
    } else {
      ...
      findings.push({
        ...
        suggestions: ['This may indicate a Percy API issue. Try again later.', 'If persistent, contact Percy support with this diagnostic output.']
      });
    }
  } catch (err) {
    findings.push({
