...
export const REQUIRED_DOMAINS = [{
  ...
  label: 'BrowserStack API',
  url: 'https://www.browserstack.com'
}, {
  label: 'BrowserStack Automate',
  url: 'https://hub.browserstack.com'
}, {
  label: 'Chromium download CDN',
  url: 'https://storage.googleapis.com',
  optional: true,
  onFail: ['Percy downloads Chromium from storage.googleapis.com for visual asset capture.', 'To fix: set PERCY_BROWSER_EXECUTABLE=/path/to/chrome to use an installed browser,', 'or mirror the Chromium CDN: export PERCY_CHROMIUM_BASE_URL=https://your-mirror/chromium/']
}];
 ...
 */
var _ConnectivityChecker_brand = /*#__PURE__*/new WeakSet();
export class ConnectivityChecker {
  constructor() {
     ...
     */
    _classPrivateMethodInitSpec(this, _ConnectivityChecker_brand);
  }
  /**
  ...
  async checkConnectivityAndSSL(options = {}) {
    ...
    const rawFindings = await Promise.all(REQUIRED_DOMAINS.map(async ({
      label,
      url,
      optional,
      onFail
    }) => {
      const finding = await _assertClassBrand(_ConnectivityChecker_brand, this, _probeTarget).call(this, label, url, proxyUrl, timeout);
    ...
    const sslFindings = _assertClassBrand(_ConnectivityChecker_brand, this, _buildSSLFindings).call(this, percyFinding === null || percyFinding === void 0 ? void 0 : percyFinding.directResult);
    return {
      connectivityFindings: rawFindings,
      sslFindings
    };
  }
}
function _buildSSLFindings(percyProbeResult) {
  ...
  if (process.env.NODE_TLS_REJECT_UNAUTHORIZED === '0') {
    findings.push({
      ...
      suggestions: ['SSL check results may not reflect your actual certificate status.', 'This is typically a temporary workaround. Remove after diagnosing the underlying SSL issue.', 'Consider using NODE_EXTRA_CA_CERTS=/path/to/ca.crt instead for a permanent fix.']
    });
  }
  if (!percyProbeResult) {
    findings.push({
      status: 'skip',
      message: 'SSL check skipped — percy.io was not probed.'
    });
    return findings;
  }
  if (httpProber.isSslError(percyProbeResult)) {
    findings.push({
      ...
      suggestions: ['Your network proxy or VPN may be intercepting HTTPS traffic with its own certificate.', 'Ask your network admin to add percy.io to the SSL inspection exclusion list.', 'Solution: export NODE_TLS_REJECT_UNAUTHORIZED=0  (re-run percy doctor to verify)', 'Or trust the proxy CA: export NODE_EXTRA_CA_CERTS=/path/to/proxy-ca.crt']
    });
...
async function _probeTarget(label, url, proxyUrl, timeout) {
  ...
  const direct = await httpProber.probeUrl(url, {
    timeout
  });

  // Proxy probe (only if a proxy is configured)
  const viaProxy = proxyUrl ? await httpProber.probeUrl(url, {
    proxyUrl,
    timeout
  }) : null;
  ...
  if (httpProber.isSslError(direct)) {
    return {
      ...
      suggestions: ['See the SSL / TLS section above for remediation steps.', 'Ensure your network proxy is not performing SSL inspection on percy.io / browserstack.com.', 'Solution: export NODE_TLS_REJECT_UNAUTHORIZED=0']
    };
  }
  ...
  if (proxyReachable) {
    return {
      ...
      suggestions: [`Ensure the proxy server is configured: set HTTPS_PROXY=${proxyUrl}`, 'Add proxy settings to your Percy config file under the proxy key.', `Direct error: ${direct.errorCode ?? direct.error}`]
    };
  }
  ...
  return {
    ...
    suggestions: [`Ensure your network allows outbound HTTPS to ${new URL(url).hostname}.`, 'Contact your network/IT team to whitelist: percy.io, www.browserstack.com, hub.browserstack.com.', 'If behind a corporate proxy, set HTTPS_PROXY=http://proxy-host:port in your environment.']
  };
}