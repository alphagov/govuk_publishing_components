...
export async function checkConfig(options = {}) {
  var _result, _process$env$PERCY_TO;
  const {
  ...
  try {
    ...
    findings.push({
      ...
      suggestions: ['Check for YAML/JSON syntax errors in your config file.', 'Run: percy config:validate for detailed error output.']
    });
    return findings;
  }
  if (!((_result = result) !== null && _result !== void 0 && _result.config)) {
    findings.push({
      category: 'config_not_found',
      status: 'info',
      message: 'No Percy configuration file detected.',
      suggestions: ['Percy works with default settings when no config file is present.', 'Create .percy.yml to customize snapshot widths, CSS, and other options.']
    });
    return findings;
  }
  ...
  if (token && result.config) {
    ...
    const automateOnlyKeys = ['fullPage', 'freezeAnimation', 'freezeAnimatedImage', 'freezeAnimatedImageOptions', 'ignoreRegions', 'considerRegions', 'scaleToFit'];
    ...
    if (!isAutomate) {
      ...
      if (mismatched.length > 0) {
        findings.push({
          ...
          suggestions: ['These config keys will be silently ignored for your project type.', 'Remove them from your config or use an Automate project token.']
        });
      }
    }
    if (projectType !== 'web') {
      ...
      if (mismatched.length > 0) {
        findings.push({
          ...
          suggestions: ['These config keys will be silently ignored for your project type.', 'Remove them from your config or use a Web project token.']
        });
      }
    }
  }
