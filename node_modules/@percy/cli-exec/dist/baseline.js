...
function findPercyPackages(dir, log) {
  try {
    ...
    log === null || log === void 0 || log.debug(`Baseline provider discovery walk failed: ${err.message}`);
    return [];
  }
}
...
export async function findBaselineProvider({
  cwd = process.cwd(),
  log
} = {}) {
  if (process.env.PERCY_DROPIN_DISABLE === 'true') {
    log === null || log === void 0 || log.debug('Drop-in disabled via PERCY_DROPIN_DISABLE — skipping baseline provider discovery');
    return null;
  }
  for (let pkgPath of findPercyPackages(cwd, log)) {
    ...
    try {
      ...
      let providerPath = (_pkg$PercyCli = pkg['@percy/cli']) === null || _pkg$PercyCli === void 0 ? void 0 : _pkg$PercyCli.baselineProvider;
      ...
      if (!resolved.startsWith(path.resolve(sanitizePath(pkgPath)) + path.sep)) {
        log === null || log === void 0 || log.debug(`Skipping baseline provider from ${pkgPath}: provider path escapes the package`);
        continue;
      }
      ...
      log === null || log === void 0 || log.debug(`Skipping baseline provider from ${pkgPath}: ${err.message}`);
    }
  }
  return null;
}
...
function isAuthFailure(error) {
  var _error$response;
  let status = (_error$response = error.response) === null || _error$response === void 0 ? void 0 : _error$response.statusCode;
...
export async function waitForSeedBuild(client, buildId, {
  log,
  timeout = 600000,
  interval = 5000
}) {
  ...
  for (;;) {
    try {
      ({
        state
      } = (await client.getBuild(buildId)).data.attributes);
    } catch (err) {
    ...
    log[polls++ % 6 === 0 ? 'info' : 'debug'](`Baseline build still ${state} — waiting for it to finish before tests start`);
...
export async function maybeSeedBaseline(percy, provider, {
  log,
  waitTimeout,
  waitInterval
}) {
  try {
    var _res$data, _res$data$attributes;
    ...
    let {
      ...
      degraded,
      reason
    } = (await provider.discoverBaselines({
      cwd: process.cwd(),
      log
    })) || {};
    if (degraded) {
    ...
    if (!(res !== null && res !== void 0 && (_res$data = res.data) !== null && _res$data !== void 0 && _res$data.id) || ((_res$data$attributes = res.data.attributes) === null || _res$data$attributes === void 0 ? void 0 : _res$data$attributes['build-number']) !== 1) {
      log.info(`Found ${baselines.length} committed baseline snapshot(s), but this project ` + 'already has builds — to (re)establish the baseline from them, run: ' + 'npx percy playwright:setup-baseline');
      return false;
    }
    let buildId = res.data.id;
    log.info(`New Percy project with ${baselines.length} committed baseline snapshot(s) ` + 'detected — establishing your baseline (build #1) before running tests');
    ...
    if (seeded === 0) {
      log.warn(`Could not upload any of the ${baselines.length} committed baseline snapshot(s) — ` + 'baseline not established. Once connectivity recovers, run: ' + 'npx percy playwright:setup-baseline');
      return false;
    }
    ...
    if (state === 'finished') {
      log.info(`Baseline established from ${seeded}/${baselines.length} committed snapshot(s) ` + 'and auto-approved — this run diffs against it.');
      ...
      log.warn(`Uploaded ${seeded}/${baselines.length} baseline snapshot(s), but this token ` + 'cannot read build status, so Percy cannot wait for your baseline to finish before ' + 'tests start. Use your project\'s FULL ACCESS token as PERCY_TOKEN for this first run ' + '(Project settings → Tokens). If snapshots in this run appear as new instead of ' + 'diffing, approve build #1 in the dashboard.');
    } else {
      log.warn(`Baseline build did not finish processing in time (state: ${state || 'unknown'}) — ` + 'snapshots in this run may show as new instead of diffing against the baseline');
    }
    return true;
  } catch (err) {
...
async function imageSnapshotResources({
  name,
  filepath,
  width,
  height
...
export async function uploadBaselines(client, buildId, baselines, {
  log,
  projectType = 'web'
}) {
  let queue = [...baselines];
  ...
  await Promise.all(Array.from({
    length: Math.min(SEED_CONCURRENCY, baselines.length)
  }, worker));
  return seeded;
}