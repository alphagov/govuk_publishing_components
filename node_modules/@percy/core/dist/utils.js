...
export function processCorsIframesInDomSnapshot(domSnapshot, rootUrl) {
  ...
  for (const frame of crossOriginFrames) {
    ...
    if (isHttpOrHttpsUrl(frameUrl)) {
      ...
      resourceUrl = domSnapshot.width ? appendUrlSearchParam(frameUrl, 'percy_width', domSnapshot.width) : frameUrl;
    } else {
      const syntheticUrl = buildSyntheticFrameResourceUrl(rootUrl, iframeData === null || iframeData === void 0 ? void 0 : iframeData.percyElementId);
      if (!syntheticUrl) {
        logger('core:utils').debug(`Skipping corsIframes entry with non-http(s) frameUrl and no percyElementId: ${frameUrl}`);
        continue;
      }
      resourceUrl = domSnapshot.width ? appendUrlSearchParam(syntheticUrl, 'percy_width', domSnapshot.width) : syntheticUrl;
...
export function buildImageSnapshotHtml({
  name,
  imageUrl,
  width,
  height
...
export function createImageSnapshotResources({
  ...
  return [createRootResource(rootUrl, buildImageSnapshotHtml({
    name,
    imageUrl,
    width,
    height
