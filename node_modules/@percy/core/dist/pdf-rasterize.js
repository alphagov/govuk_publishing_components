...
async function createAssetServer(pdfBuffer, assets) {
  ...
  server.route('get', '/doc.pdf', (req, res) => res.send(200, 'application/pdf', pdfBuffer));
  server.route('get', '/', (req, res) => res.send(200, 'text/html', '<!doctype html><html><head><meta charset="utf-8"></head><body></body></html>'));
  await server.listen();
  return server;
}
...
export function withTimeout(promise, ms, description) {
  ...
  let timeout = new Promise((resolve, reject) => {
    timer = setTimeout(() => reject(new Error(`Timed out after ${ms}ms ${description}`)), ms);
    timer.unref();
  });
...
export async function rasterizePdf(percy, pdfBuffer, options) {
  ...
  if (!Number.isFinite(scale) || scale <= 0 || scale > MAX_SCALE) {
    throw asInputError(new Error(`Invalid scale ${options.scale}: expected a number between 0 and ${MAX_SCALE}`));
  }
  ...
  try {
    ...
    await withTimeout(/* eslint-disable-next-line no-new-func */
    page.eval(new Function(pdfjsSource)), PAGE_RENDER_TIMEOUT, 'injecting pdf.js');
    let {
      pageCount
    } = await withTimeout(page.eval(openDocument, {
      origin
    ...
    let sizes = await withTimeout(page.eval(measurePages, {
    ...
    for (let {
      ...
      if (effectiveScale < scale) {
        log.warn(`Page ${pageNumber} is ${Math.round(width)}x${Math.round(height)}pt; ` + `scale reduced from ${scale} to ${effectiveScale.toFixed(3)} to stay within ` + 'Percy\'s 2000px limit');
      }
      let rendered = await withTimeout(page.eval(renderPage, {
        pageNumber,
    ...
    await withTimeout(page.eval(destroyDocument), PAGE_RENDER_TIMEOUT, 'closing the PDF');
    return {
      pageCount,
      scale,
      pages
    ...
    for (let result of await Promise.allSettled([(_page = page) === null || _page === void 0 ? void 0 : _page.close(), server.close()])) {
      var _page;
      /* istanbul ignore next: both closes resolve in every reachable test path */
      if (result.status === 'rejected') {
        var _result$reason;
        log.debug(`PDF cleanup failed: ${((_result$reason = result.reason) === null || _result$reason === void 0 ? void 0 : _result$reason.message) ?? result.reason}`);
      }
    }
  }
}