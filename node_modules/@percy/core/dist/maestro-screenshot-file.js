...
export async function locateScreenshot({
  ...
  if (filePath) {
    ...
    try {
      ...
      files = await glob(searchPattern, scopedGlob ? {
        dot: true,
        deep: MAX_SEARCH_DEPTH
      } : undefined);
    } catch {
      ...
      files = selfHosted ? [] : await manualScreenshotWalk(platform, sessionId, name, recursiveScope ? scopeRoot : null);
    }
  ...
  if (!realPathFwd.startsWith(`${realPrefixFwd}/`)) {
    const scopeLabel = selfHosted ? 'PERCY_MAESTRO_SCREENSHOT_DIR' : recursiveScope ? 'PERCY_MAESTRO_BS_SCOPE_ROOT' : 'session dir';
