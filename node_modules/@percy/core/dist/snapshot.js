...
import { request, hostnameMatches, yieldTo, snapshotLogName, decodeAndEncodeURLWithLogging, compareObjectTypes, normalizeOptions, base64encode, redactSecrets } from './utils.js';
...
export async function uploadSnapshotLog(percy, buildId, snapshotId, meta = {}) {
  try {
    ...
    percy.log.debug(`Snapshot log sent for ${(_meta$snapshot = meta.snapshot) === null || _meta$snapshot === void 0 ? void 0 : _meta$snapshot.name}`, meta);
  } catch (err) {
    var _meta$snapshot2;
    percy.log.debug(`Could not send the snapshot log for ${(_meta$snapshot2 = meta.snapshot) === null || _meta$snapshot2 === void 0 ? void 0 : _meta$snapshot2.name}`, meta);
    percy.log.debug(err);
  } finally {
