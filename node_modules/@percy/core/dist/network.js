...
function isHttpsUpgradeFallback(from, to) {
  return from.startsWith('https://') && to.startsWith('http://') && from.slice('https:'.length) === to.slice('http:'.length);
}
...
export function originURL(request) {
  ...
  for (let i = 0; i < chain.length; i++) {
    ...
    let next = normalizeURL(((_chain = chain[i + 1]) === null || _chain === void 0 ? void 0 : _chain.url) ?? request.url);
...
async function captureResourceDirectly(network, request, session) {
  ...
  try {
    ...
    let {
      ...
      status,
      headers: responseHeaders
    } = await raceWithTimeout(makeDirectRequest(network, request, session), timeoutMs, `Direct fetch timed out after ${timeoutMs}ms`);
...
async function saveResponseResource(network, request, session) {
  ...
  if (!resource || !resource.root && !resource.provided && disableCache) {
    try {
      ...
      let body = shouldCapture && (await raceWithTimeout(response.buffer(), process.env.PERCY_GZIP ? 60000 : 30000, `response.buffer() timed out for ${url}`));
      ...
      logAssetInstrumentation(log, 'asset_load_missing', 'network_error', {
        url,
        snapshot: meta.snapshot,
        requestType: request === null || request === void 0 ? void 0 : request.type,
        error: error.message
      });
    }
  }
