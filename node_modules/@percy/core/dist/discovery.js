...
import { normalizeURL, hostnameMatches, createResource, createRootResource, createPercyCSSResource, yieldAll, snapshotLogName, waitForTimeout, withRetries, waitForSelectorInsideBrowser, isGzipped, maybeScrollToBottom, assertNotMetadataTarget } from './utils.js';
    ...
    for (let index = 0; index < resources.length; index++) {
      ...
      try {
        ...
        logAssetInstrumentation(log, 'asset_load_missing', 'network_error', {
          url: resource.url,
          snapshot: (_snapshot$meta = snapshot.meta) === null || _snapshot$meta === void 0 ? void 0 : _snapshot$meta.snapshot,
          error: error.message
        });
      ...
      if (size > MAX_RESOURCE_SIZE && !resource.root && !resource.log) {
        ...
        logAssetInstrumentation(log, 'asset_not_uploaded', 'resource_too_large', {
          url: resource.url,
          size,
          snapshot: (_snapshot$meta2 = snapshot.meta) === null || _snapshot$meta2 === void 0 ? void 0 : _snapshot$meta2.snapshot
        });
        log.debug(`- Skipping resource larger than allowed size after gzip (${size} bytes)`, {
          url: resource.url
        });
        continue;
      }
  ...
  return queue.set({
    ...
    if (configuredMaxCacheRamMB != null) {
      if (configuredMaxCacheRamMB < minCacheRamMB) {
        percy.log.warn(`--max-cache-ram=${configuredMaxCacheRamMB}MB is below the ${minCacheRamMB}MB minimum ` + `(individual resources up to ${minCacheRamMB}MB would otherwise be dropped). ` + `Continuing with the minimum: ${minCacheRamMB}MB.`);
