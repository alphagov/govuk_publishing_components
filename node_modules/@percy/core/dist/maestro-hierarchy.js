...
function findStatusBarFrameHeight(node) {
  ...
  if (node.elementType === IOS_STATUS_BAR_ELEMENT_TYPE) {
    ...
    return ((_node$frame = node.frame) === null || _node$frame === void 0 ? void 0 : _node$frame.Height) || null;
  }
...
async function deriveIosInsets({
  port,
  pngDims,
  httpRequest,
  sessionId
  ...
  const rootPointHeight = aut === null || aut === void 0 || (_aut$frame = aut.frame) === null || _aut$frame === void 0 ? void 0 : _aut$frame.Height;
...
async function deriveAndroidInsets({
  execAdb,
  getEnv
}) {
...
export async function deriveDeviceInsets(options) {
  ...
  try {
    ...
    return await deriveAndroidInsets({
      execAdb,
      getEnv
    });
  } catch {
...
export async function dump(options) {
  ...
  if (platform === 'ios') {
    ...
    if (!driverHostPortRaw) {
      const probe = await runIosHttpDump({
        port: 7001,
        sessionId,
        httpRequest
      });
      if (probe.kind === 'hierarchy' || probe.kind === 'no-aut-tree') {
        ...
        if (probe.kind === 'hierarchy') {
          ...
          recordResolverFinalFailure({
            platform: 'ios',
            failureClass: failureClassFromReason(probe.reason)
          });
        }
        return probe;
      }
