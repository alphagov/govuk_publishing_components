...
function inPageCeiling() {
  return Math.max(parseInt(process.env.PERCY_PAGE_LOAD_TIMEOUT, 10) || 0, parseInt(process.env.PERCY_STORY_RENDER_TIMEOUT, 10) || 0) * 2;
}
...
export function cdpTimeout() {
  ...
  Session.TIMEOUT = Number.isNaN(configured) ? Math.max(DEFAULT_CDP_TIMEOUT, inPageCeiling()) : Math.max(0, configured);
...
export function cdpTimeoutMessage(method, timeout) {
  return `Protocol error (${method}): Timed out after ${timeout}ms ` + 'waiting for a response from the browser. The browser stopped responding — a ' + 'renderer or browser process that is frozen, starved of CPU, or swapped out ' + 'never replies and never runs its own timers. Raise PERCY_CDP_TIMEOUT if this ' + 'command is legitimately slower than that, or set it to 0 to disable the deadline.';
}
