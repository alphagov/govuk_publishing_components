...
import { createImageSnapshotResources, getPackageJSON, normalizeOptions } from './utils.js';
...
export async function loadPdfModule(load = () => import('@percy/cli-pdf')) {
  try {
    ...
    throw new ServerError(501, ['PDF snapshots require the @percy/cli-pdf package, which is not installed.', 'Install it with: npm install --save-dev @percy/cli-pdf', `(underlying error: ${error.message})`].join(' '));
  }
}
...
export async function handlePdfSnapshot(req, res, percy) {
  ...
  let {
    ...
    scale,
    ...rest
  } = validatePdfSnapshotOptions(body);
  ...
  let rasterized;
  try {
    rasterized = await rasterizePdf(percy, buffer, {
      pages,
      excludePages,
      scale
    });
    ...
    let message = ((error === null || error === void 0 ? void 0 : error.message) ?? String(error)).split('\n')[0];
    log.error(`Failed to rasterize PDF "${name}": ${message}`);
    throw new ServerError((error === null || error === void 0 ? void 0 : error.status) ?? 500, `Could not rasterize PDF: ${message}`);
  }
  let {
    pageCount,
    pages: rendered
  ...
  log.info(`PDF "${name}": snapshotting ${rendered.length} of ${pageCount} page(s)` + (sync ? ' (waiting for comparison results)' : ''));
  let queued = queuePages(percy, {
    name,
    rendered,
    sync,
    snapshotOptions
  ...
  let results = await Promise.all(queued.map(async ({
    page,
    snapshotName,
    promise
  }) => ({
