...
function defaultExecMaestro() {
  return spawnSync('maestro', ['--version'], {
    encoding: 'utf8'
  });
}
...
function detectMaestroVersion(execMaestro, log) {
  ...
  try {
    ...
    log === null || log === void 0 || log.debug(`maestro --version failed (${err.code || err.message}); skipping --driver-host-port injection`);
    return null;
  }
  if (!result || result.error || result.status !== 0) {
    log === null || log === void 0 || log.debug('maestro --version returned no usable output; skipping --driver-host-port injection');
    return null;
  }
  const out = `${result.stdout || ''}${result.stderr || ''}`;
  const m = out.match(/(\d+)\.(\d+)/);
  if (!m) {
    log === null || log === void 0 || log.debug('could not parse maestro version; skipping --driver-host-port injection');
    return null;
  }
  return {
    major: Number(m[1]),
...
export async function maybeInjectDriverHostPort(ctx, log, deps = {}) {
  ...
  if (!version) return;
  if (version.major < DRIVER_HOST_PORT_MIN_MAJOR || version.major === DRIVER_HOST_PORT_MIN_MAJOR && version.minor < DRIVER_HOST_PORT_MIN_MINOR) {
    log === null || log === void 0 || log.debug(`Maestro ${version.major}.${version.minor} predates --driver-host-port ` + '(>= 2.6.0); relying on the 127.0.0.1:7001 relay probe');
    return;
  }
  ...
  if (port === null) {
    ...
    try {
      ...
      log === null || log === void 0 || log.warn(`[percy] could not reserve a free port for --driver-host-port (${err.message}); ` + 'relying on the 127.0.0.1:7001 relay probe');
      return;
    }
  }
  args.splice(testIdx + 1, 0, '--driver-host-port', String(port));
  process.env.PERCY_IOS_DRIVER_HOST_PORT = String(port);
  log === null || log === void 0 || log.info(`[percy] iOS Maestro driver port prescribed → ${port} (Maestro ${version.major}.${version.minor})`);
}