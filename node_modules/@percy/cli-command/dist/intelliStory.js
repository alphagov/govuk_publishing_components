...
export function resolvePattern(raw, projectRoot, invocationDir) {
  ...
  if (raw.startsWith(ROOT_DIR_TOKEN)) {
    return {
      raw,
      rebased: toPosix(raw.slice(ROOT_DIR_TOKEN.length)),
      anchored: true
    };
  }
...
function rootReadingHit(affectedNodes, {
  raw,
  rebased,
  anchored
}) {
...
const isExcluded = (relPath, configDirs) => isInsideDirs(relPath, ['node_modules', ...configDirs]);
...
function transformModule(m, fileIndex, projectRoot, configDirs) {
  ...
  if (typeof out.id === 'string') {
    return {
      dropped: path.isAbsolute(stripNull(m.id)) ? 'excluded' : 'unresolved'
    };
  }
  const mapEntry = e => {
    const copy = {
...
function readStats(statsFile, projectRoot, log, configDirs) {
  ...
  for (const m of rawModules) {
    const {
      module,
      dropped
    } = transformModule(m, fileIndex, projectRoot, configDirs);
    if (module) modules.push(module);else if (dropped === 'unresolved') unresolved++;else excluded++;
  }
  if (excluded) log === null || log === void 0 || log.debug(`IntelliStory: skipped ${excluded} module(s) in node_modules or the Storybook config directory`);
  ...
  if (unresolved) {
    ...
    log === null || log === void 0 || log.warn(`IntelliStory: dropped ${unresolved} of ${candidates} module(s) with unresolved (non-absolute) ids; the dependency graph may be missing edges for them`);
  }
  const files = [...fileIndex.entries()].sort((a, b) => a[1] - b[1]).map(([p]) => p);
  return {
    files,
    modules,
...
function isNotAllowed(e) {
  var _e$response;
  return (e === null || e === void 0 || (_e$response = e.response) === null || _e$response === void 0 ? void 0 : _e$response.statusCode) === 403;
}
function apiBailError(e, action) {
  ...
  return new IntelliStoryBailError(`IntelliStory: ${action} failed: ${e === null || e === void 0 ? void 0 : e.message}; running full snapshot set`);
}
async function pollGraphStatus(percy, buildId, log) {
  for (let i = 0; i < POLL_ATTEMPTS; i++) {
    ...
    try {
      ...
      log.debug(`IntelliStory: graph status (attempt ${i + 1}) errored: ${e === null || e === void 0 ? void 0 : e.message}`);
      if (i < POLL_ATTEMPTS - 1) await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
      continue;
    }
    const status = (_res = res) === null || _res === void 0 ? void 0 : _res.status;
    log.debug(`IntelliStory: graph status (attempt ${i + 1}) = ${status}`);
    if (status === 'done' || status === 'failed') return {
      status,
      data: (_res2 = res) === null || _res2 === void 0 ? void 0 : _res2.data
    };
    if (i < POLL_ATTEMPTS - 1) await new Promise(r => setTimeout(r, POLL_INTERVAL_MS));
  }
  return {
...
export async function validateAndReadStats(buildDir, statsFile, projectRoot, log, configDirs = [DEFAULT_CONFIG_DIR]) {
  ...
  const {
    ...
    modules,
    buildId
  } = await readStats(resolvedStatsPath, projectRoot, log, configDirs);
...
export async function getBaselineAndAffectedNodes(percy, baseline, log) {
  ...
  try {
    var _percy$build;
    baseLookup = await percy.client.getIntelliStorySnapshotNameToCommit((_percy$build = percy.build) === null || _percy$build === void 0 ? void 0 : _percy$build.id);
  } catch (e) {
    throw apiBailError(e, 'base build lookup');
  }
  log.debug(`IntelliStory: base lookup ${JSON.stringify(baseLookup)}`);
  if ((_baseLookup = baseLookup) !== null && _baseLookup !== void 0 && _baseLookup.browsers_changed_from_base) {
  ...
  if (baseline) {
    ...
    var _baseLookup2;
    if (!((_baseLookup2 = baseLookup) !== null && _baseLookup2 !== void 0 && _baseLookup2.base_build_commit_sha)) {
...
export function resolveConfigDirs(configDir, projectRoot, invocationDir, log) {
  ...
  const {
    rebased
  } = resolvePattern(configDir, projectRoot, invocationDir);
  if (rebased == null) {
    log === null || log === void 0 || log.warn(`IntelliStory: configDir "${configDir}" resolves outside the project root; falling back to "${DEFAULT_CONFIG_DIR}"`);
    return [...dirs];
  }
  ...
  if (!fs.existsSync(path.resolve(projectRoot, rebased))) {
    ...
    if (!path.isAbsolute(configDir) && fs.existsSync(path.resolve(projectRoot, configDir))) {
      log === null || log === void 0 || log.warn(`IntelliStory: configDir "${configDir}" does not exist relative to the invocation directory ("${describeInvocation(projectRoot, invocationDir)}") but does relative to the project root; recognising both. Write it relative to the invocation directory to make this explicit.`);
      dirs.add(asRootRelative);
    } else {
      log === null || log === void 0 || log.warn(`IntelliStory: configDir "${configDir}" resolves to "${rebased}", which does not exist; it is interpreted relative to the invocation directory, not the project root`);
    }
  }
  dirs.add(rebased);
  return [...dirs];
...
export function assertNoBailOnChanges(affectedNodes, bailOnChanges, {
  ...
  invocationDir = projectRoot,
  log
} = {}) {
  if (!(bailOnChanges !== null && bailOnChanges !== void 0 && bailOnChanges.length)) return;
  for (const raw of bailOnChanges) {
    ...
    if (!bailed) {
      ...
      if (!rootHit) continue;
      log === null || log === void 0 || log.warn(`IntelliStory: bailOnChanges pattern "${raw}" matched "${rootHit}" only when read relative to the project root. Patterns are relative to the invocation directory ("${describeInvocation(projectRoot, invocationDir)}"); bailing anyway, but write it as ${suggestRewrite(raw, projectRoot, invocationDir)} to make this explicit.`);
      bailed = rootHit;
    }
...
export function enforceUntraced(affectedNodes, untraced, {
  ...
  if (!(untraced !== null && untraced !== void 0 && untraced.length)) return affectedNodes;
  const resolved = [];
  for (const raw of untraced) {
    const r = resolvePattern(raw, projectRoot, invocationDir);
    if (r.rebased == null) {
      log === null || log === void 0 || log.warn(`IntelliStory: ignoring untraced pattern "${raw}" — it resolves outside the project root and can never match a changed file`);
      continue;
    }
    const err = patternError(r.rebased);
    if (err) {
      log === null || log === void 0 || log.warn(`IntelliStory: ignoring untraced pattern "${raw}" — not a valid glob (${err.message})`);
      continue;
    }
    const rootHit = rootReadingHit(affectedNodes, r);
    if (rootHit) {
      log === null || log === void 0 || log.warn(`IntelliStory: untraced pattern "${raw}" matches "${rootHit}" only when read relative to the project root. Patterns are relative to the invocation directory ("${describeInvocation(projectRoot, invocationDir)}") — not applied; write it as ${suggestRewrite(raw, projectRoot, invocationDir)}.`);
      continue;
    }
    resolved.push(r);
  }
  return affectedNodes.filter(p => !resolved.some(({
    rebased
  }) => matchesPattern(p, rebased)));
}
...
export function extractStorybookPaths(snapshots, normalizeImportPath, log) {
  ...
  if (storybookPaths.length === 0) {
    var _snapshots$, _snapshots$2, _snapshots$3;
    log.warn(`IntelliStory: no snapshots have importPath set — check Storybook story extraction. Sample snapshot: ${JSON.stringify({
      id: (_snapshots$ = snapshots[0]) === null || _snapshots$ === void 0 ? void 0 : _snapshots$.id,
      name: (_snapshots$2 = snapshots[0]) === null || _snapshots$2 === void 0 ? void 0 : _snapshots$2.name,
      importPath: (_snapshots$3 = snapshots[0]) === null || _snapshots$3 === void 0 ? void 0 : _snapshots$3.importPath,
      keys: snapshots[0] ? Object.keys(snapshots[0]) : []
    })}`);
  } else {
...
export async function runGraphGeneration(percy, buildId, payload, log) {
  ...
  log.debug(`IntelliStory: starting graph generation job ${JSON.stringify({
    buildId,
    files,
    modules,
    storybookPaths,
  ...
  const {
    status
  } = await pollGraphStatus(percy, buildId, log);
  if (status !== 'done') {
...
export function maybeWriteTrace(trace, data, log) {
  if (trace && data !== null && data !== void 0 && data.vertices && data !== null && data !== void 0 && data.edges && data !== null && data !== void 0 && data.transitive_closure_matrix_sparse) {
...
export async function applyIntelliStory(percy, snapshots, intelliStoryConfig, buildDir) {
  ...
  const buildId = (_percy$build2 = percy.build) === null || _percy$build2 === void 0 ? void 0 : _percy$build2.id;
  if (!buildId) {
  ...
  const {
    files,
    modules
  } = await validateAndReadStats(buildDir, statsFile, projectRoot, log, configDirs);
  let {
    baseRef,
    affectedNodes
  } = await getBaselineAndAffectedNodes(percy, baseline, log);
  ...
  const normalizeImportPath = p => {
    ...
    let rel = p;
    /* istanbul ignore next */
    if (rel.startsWith(dotPlatform)) rel = rel.slice(dotPlatform.length);else if (rel.startsWith(dotPosix)) rel = rel.slice(dotPosix.length);
  ...
  await runGraphGeneration(percy, buildId, {
    files,
    modules,
    storybookPaths,
    affectedNodes,
...
export async function writeIntelliStoryTrace(percy, intelliStoryConfig, log = logger('storybook:intelliStory')) {
  ...
  if (!trace) return;
  const buildId = (_percy$build3 = percy.build) === null || _percy$build3 === void 0 ? void 0 : _percy$build3.id;
  if (!buildId) return;
