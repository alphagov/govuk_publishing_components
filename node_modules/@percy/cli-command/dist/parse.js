...
exports.default = exports.ParseError = void 0;
exports.parse = parse;
...
class ParseError extends Error {
  constructor(...args) {
    ...
    _defineProperty(this, "name", this.constructor.name);
  }
...
async function maybeParseCommand(input, parsed) {
  ...
  let names = (_input$0$split = (_input$ = input[0]) === null || _input$ === void 0 ? void 0 : _input$.split(':')) !== null && _input$0$split !== void 0 ? _input$0$split : [];
  let command;
  ...
  if (command) {
    ...
    parsed.log = (0, _logger.default)(`cli:${command.name}`);
...
async function maybeParseFlag(input, parsed) {
  ...
  if (flag) {
    ...
    if (!flag.type || flag.type === 'boolean') {
      ...
      if (!short && value) {
        parseError(`Unexpected option argument for '${(0, _help.flagUsage)(flag)}'`);
      ...
      if (!input.length || isFlag(input[0])) {
        parseError(`Missing option argument for '${(0, _help.flagUsage)(flag)}'`);
    ...
    if (flag.multiple) {
      var _parsed$operators$get;

      value = ((_parsed$operators$get = parsed.operators.get(flag)) !== null && _parsed$operators$get !== void 0 ? _parsed$operators$get : []).concat(value);
...
async function maybeParseArg(input, parsed) {
  var _parsed$__args_index_, _parsed$command$defin;

  if (isFlag(input[0])) return; // the argument index needs to be tracked since flag options can precede argument options

  let index = (_parsed$__args_index_ = parsed.__args_index__) !== null && _parsed$__args_index_ !== void 0 ? _parsed$__args_index_ : parsed.__args_index__ = 0; // there are arguments left to parse
...
function validateInclusiveFlag(flag, parsed) {
  ...
  if (invalid.length) {
    ...
    invalid = [flag, ...invalid].map(f => (0, _help.flagUsage)(f)).join("', '");
...
function attributeName(option, parsed) {
  var _parsed$operators$get2;

  let {
    attribute,
    deprecated
  } = option;
  let value = (_parsed$operators$get2 = parsed.operators.get(option)) !== null && _parsed$operators$get2 !== void 0 ? _parsed$operators$get2 : option.default; // the option's attribute name might be contingent on it's value
  ...
  name || (name = option.name ? (0, _normalize.camelcase)(option.name) : option.short); // deprecated options can be mapped to other options
  ...
  if (deprecated) {
    ...
    if ((_recommend = recommend) !== null && _recommend !== void 0 && _recommend.startsWith('--')) {
      ...
      recommend = opt ? `Use '${(0, _help.flagUsage)(opt)}' instead.` : '';
    ...
    if (parsed.operators.has(option)) {
      ...
      let flag = parsed.command.definition.flags.includes(option);
      let usage = flag ? (0, _help.flagUsage)(option) : option.name;
...
async function parse(command, argv) {
  ...
  for (let arg of (_parsed$command$defin2 = parsed.command.definition.args) !== null && _parsed$command$defin2 !== void 0 ? _parsed$command$defin2 : []) {
    var _parsed$command$defin2;
