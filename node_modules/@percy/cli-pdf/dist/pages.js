...
export function resolvePages({
  ...
  if (outOfRange.length) {
    throw new Error(`Requested page${outOfRange.length > 1 ? 's' : ''} ${outOfRange.join(', ')} ` + `but the document has only ${pageCount} page${pageCount > 1 ? 's' : ''}`);
  }
  let resolved = [...new Set(selected)].filter(p => !excluded.has(p)).sort((a, b) => a - b);
  if (!resolved.length) {
    throw new Error('No pages left to snapshot after applying `pages` and `excludePages`');
  }
  if (resolved.length > MAX_PAGES) {
    throw new Error(`Requested ${resolved.length} pages but the maximum per request is ${MAX_PAGES}. ` + 'Narrow the selection with `pages` (for example "1-100") and issue several requests.');
  }
  return resolved;
}
