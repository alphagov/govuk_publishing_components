...
export function fitScale(requestedScale, {
  width,
  height
}) {
...
export function assertRasterDimensions(pageNumber, width, height) {
  if (width < MIN_DIMENSION || height < MIN_DIMENSION) {
    throw new Error(`Page ${pageNumber} rasterized to ${width}x${height}px, below Percy's ` + `${MIN_DIMENSION}px minimum. Increase \`scale\`.`);
  }
}
export async function openDocument(_, {
  origin
}) {
...
export async function measurePages(_, {
  pageNumbers
}) {
  let {
    doc
...
export async function renderPage(_, {
  pageNumber,
  scale
}) {
  let {
...
export async function destroyDocument() {
  ...
  if (state !== null && state !== void 0 && state.doc) {
