...
class Project {
    static async find(configuration, startingCwd) {
        ...
        const packageCwdLog = formatUtils.pretty(configuration, fslib_1.ppath.relative(project.cwd, packageCwd), formatUtils.Type.PATH);
    ...
    async setupResolutions() {
        ...
        this.storedPackages = new Map();
        this.lockFileChecksum = null;
        const lockfilePath = fslib_1.ppath.join(this.cwd, fslib_1.Filename.lockfile);
    ...
    tryWorkspaceByCwd(workspaceCwd) {
        ...
        workspaceCwd = (fslib_1.ppath.normalize(workspaceCwd).replace(/\/+$/, ``) || fslib_1.PortablePath.root);
    ...
    async loadUserConfig() {
        ...
        //
        const pnpPath = fslib_1.ppath.join(this.cwd, `.pnp.cjs`);
        if (await fslib_1.xfs.existsPromise(pnpPath))
            miscUtils.dynamicRequire(pnpPath).setup();
        const configPath = fslib_1.ppath.join(this.cwd, `yarn.config.cjs`);
        if (!await fslib_1.xfs.existsPromise(configPath))
            return null;
    ...
    async fetchEverything({ cache, report, fetcher: userFetcher, mode, persistProject = true }) {
        ...
        if (report.cacheMisses.size > 0 || cleanInfo) {
            const addedSizes = await Promise.all([...report.cacheMisses].map(async (locatorHash) => {
                ...
                const stat = await fslib_1.xfs.statPromise(p);
                return stat.size;
            }));
            ...
            for (const locatorHash of buildablePackages) {
                ...
                const pkgBuilds = buildInfo.buildLocations.map(async (location) => {
                    ...
                    for (const directive of buildInfo.buildDirectives) {
                        ...
                        const stdin = null;
                        const wasBuildSuccessful = await fslib_1.xfs.mktempPromise(async (logDir) => {
                            ...
                            if (StreamReport_1.SUPPORTS_GROUPS)
                                report.reportFold(fslib_1.npath.fromPortablePath(logFile), fslib_1.xfs.readFileSync(logFile, `utf8`));
                            return isOptional;
                        });
    ...
    async install(opts) {
        ...
        await opts.report.startTimerPromise(`Post-resolution validation`, {
            ...
            if (initialLockfile !== null) {
                const newLockfile = (0, fslib_1.normalizeLineEndings)(initialLockfile, this.generateLockfile());
    ...
    async persistLockfile() {
        ...
        let currentContent = ``;
        try {
            currentContent = await fslib_1.xfs.readFilePromise(lockfilePath, `utf8`);
        }
        catch {
            // ignore errors, no big deal
        }
        const newContent = this.generateLockfile();
        const normalizedContent = (0, fslib_1.normalizeLineEndings)(currentContent, newContent);
        if (normalizedContent === currentContent)
            return;
        await fslib_1.xfs.writeFilePromise(lockfilePath, normalizedContent);
    ...
    async persistInstallStateFile() {
        ...
        await fslib_1.xfs.writeFilePromise(installStatePath, await gzip(serializedState));
    ...
    async restoreInstallState({ restoreLinkersCustomData = true, restoreResolutions = true, restoreBuildState = true } = {}) {
        ...
        let installState;
        try {
            const installStateBuffer = await gunzip(await fslib_1.xfs.readFilePromise(installStatePath));
    ...
    async cacheCleanup({ cache, report }) {
        ...
        if (!(0, folderUtils_1.isFolderInside)(cache.cwd, this.cwd))
            return null;
        if (!(await fslib_1.xfs.existsPromise(cache.cwd)))
            return null;
        ...
        for (const entry of await fslib_1.xfs.readdirPromise(cache.cwd)) {
            ...
            if (cache.markedFiles.has(entryPath))
                continue;
            if (cache.immutable) {
                report.reportError(MessageName_1.MessageName.IMMUTABLE_CACHE, `${formatUtils.pretty(this.configuration, fslib_1.ppath.basename(entryPath), `magenta`)} appears to be unused and would be marked for deletion, but the cache is immutable`);
            }
            else {
                cleanupPromises.push(fslib_1.xfs.lstatPromise(entryPath).then(async (stat) => {
