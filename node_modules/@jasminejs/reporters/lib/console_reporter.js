...
class ConsoleReporter {
  ...
  configure(options) {
    ...
    /**
     * Whether to list each spec that was marked not applicable rather than
     * just summarizing them.
     @name ConsoleReporterOptions#this.#listNotApplicableSpecs
     @type Boolean|undefined
     @default false
     */
    if (options.listNotApplicableSpecs !== undefined) {
      this.#listNotApplicableSpecs = options.listNotApplicableSpecs;
    }
  }
  ...
  jasmineDone(result) {
    ...
    if (this.#notApplicableSpecs.length > 0) {
      ...
      for (const spec of this.#notApplicableSpecs) {
        const r = spec.notApplicableReason;
        const n = spec.fullName;
      ...
      for (const [reason, specNames] of Object.entries(byReason)) {
        if (specNames.length === 1) {
          this.#print('1 spec');
        } else {
          this.#print(`${specNames.length} specs`);
        }

        this.#print(` not applicable because: ${reason}`);
        this.#printNewline();

        if (this.#listNotApplicableSpecs) {
